...
from ML.models.FCNN import FCNN
...
# the following 3 functions depend heaviliy on the exact ML ML used, so it is much easier to modify them on the go rather than making them very flexible
def plot_NN_bijection(dataset, xlim=(-11, 4), ylim=(-4, 7), s=2, alpha=0.3):
    model = FCNN(2)
    model.load_state_dict(torch.load("ML/saved/model_fcnn_mcml_frame_inverse"))
    model.eval()
    prediction = None
    with torch.no_grad():
        y = torch.cat(
...
#forest = KNN()
#forest.load("ML/scipy_saved_models/knn_F")
